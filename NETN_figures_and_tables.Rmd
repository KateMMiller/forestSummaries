...
```{r include = F, cache = F}
source('source_script_NETN.R') # sets up parameters. 
```

<!-- ++++ Ended update here ++++ -->
```{r include = F, cache = F}
source('./scripts/forest_summary_code_NETN.R') # main figures and tables
```
```{r include = F, cache = F}
...
cap_t3 <- if(park %in% "MORR"){
    paste0("<p style='font-size:10px; padding:0 5px;'><b>Table 3.</b> Number of plots out of ", num_plots, " where priority invasive species were detected. ", "Priority invasive species are those that capable of dominating Eastern US forest habitats. Note that Plot 14 and 25 are in a deer exclosure, and Plot 14 was not part of the original sample design. ", paste0(cycle_labs, collapse = "; "), "</p>")
} else if(park %in% c("MIMA", "ROVA", "SARA")){
...
```{r include = F, eval = T, results = 'hide'}
# Table 3 setup
tab3_cols <- if(park %in% c("MIMA", "MORR", "ROVA", "SARA")){c("Latin Name", "Common Name", 
                                  gsub("cycle_", "Cycle ", names(inv_spp[,4:ncol(inv_spp)])))
  } else {c("Latin Name", "Common Name", "Invasive", 
            gsub("cycle_", "Cycle ", names(inv_spp[,4:ncol(inv_spp)])))}

inv_spp <- if(park %in% c("MIMA", "MORR", "ROVA", "SARA")){
...
tab3 <- 
  if(park %in% c("MIMA", "MORR", "ROVA", "SARA")){
...
tab1_evalb <- ifelse(park %in% c("ACAD", "ROVA", "SARA"), TRUE, FALSE)
tab1_evalc <- ifelse(park %in% "ACAD", TRUE, FALSE)
```
