...
mor_rec_spp_long <- mor_rec2 %>% 
  select(spp_grp, 
         mortBA12, mortBA23, mortBA34, mortBA45, 
         mortStem12, mortStem23, mortStem34, mortStem45,
         recrBA12, recrBA23, recrBA34, recrBA45,
         recrStem12, recrStem23, recrStem34, recrStem45
  ) %>% 
...
p1 <- ggplot(mor_rec_spp_nz %>% filter(unit_type == "BA" & rate_type == 'recr'), 
  ...
  scale_fill_manual(values = c("2" = "#CAD7D0", 
                               ...
                               "4" = "#4D6A5A",
                               "5" = "#3E4F46"),
                    labels = c("1 \U2013 2", "2 \U2013 3", "3 \U2013 4", "4 \U2013 5"),
                    name = 'Recr. by cycle')
...
svg(paste0(new_path, "figures/", "Figure_4_", park, "_treeBA_by_species_cycle.svg"),
...
svg(paste0(new_path, "figures/", "Figure_4_", park, "_treeStem_by_species_cycle.svg"),
    height = 5.2, width = 9.5)
...
mor_rec_long <- trees_wide3 %>% group_by(ParkUnit, Plot_Name) %>% 
  summarize(c1_live_BA = sum(BA_m2ha_1_alive, na.rm = T), 
            ...
            c5_live_BA = sum(BA_m2ha_5_alive, na.rm = T),
            .groups = 'drop') %>% 
...
mor_rec_all <- mor_rec %>% group_by(ParkUnit) %>% 
  summarize(
    ...
    mortBA45 = -1 * sum(c3_4_mort_BA_yr)/sum(c3_live_BA) * 100,
    mortStem45 = -1 * sum(c3_4_mort_stem_yr)/sum(c3_live_stem) * 100,
    recrBA45 = sum(c3_4_recr_BA_yr)/sum(c3_live_BA) * 100,
    recrStem45 = sum(c3_4_recr_stem_yr)/sum(c3_live_stem) * 100,
    growBA45 = sum(c3_4_incgr_BA_yr)/sum(c3_live_BA) * 100,
