...
Pinus_spp <- c("Pinus resinosa", "Pinus", "Pinus rigida")
...
Other_Native <- c('Alnus serrulata',
                  'Amelanchier',  'Amelanchier arborea', 'Amelanchier laevis',
                  'Celtis occidentalis', 'Cladrastis kentukea', 'Juglans nigra',
                  'Nyssa sylvatica', 
                  'Tilia americana',
                  'Picea rubens', 
                  'Platanus occidentalis', 
                  'Salix', 'Salix discolor', 'Unknown Conifer',
...
Exotic_spp <- c('Aesculus hippocastanum', 'Ailanthus altissima',
                'Catalpa speciosa', 'Cladrastis kentukea',
                'Crataegus',  
...
trees <- trees %>% 
  mutate(spp_grp = case_when(ScientificName %in% Acer_spp ~ "Acer spp. (maple)",
                             ...
                             ScientificName %in% c(Other_Native, Populus_spp, Pinus_spp, # doesn't include PINSTR
                                                   'Betula populifolia') ~ "Other Native spp.",
...
net_stems <- 
  ...
  scale_color_manual(values = c(
    "Acer platanoides (Norway maple)" = '#9D0909',
    "Acer spp. (maple)" = "#54FF00",
    "Betula lenta (sweet birch)" = "#38A800",
#    "Betula populifolia (gray birch)" = "#D9E981",
...
Betula_spp <- c('Betula','Betula alleghaniensis', #'Betula lenta',  
                'Betula X cearulea',
                'Betula papyrifera', 'Betula populifolia', 'Betula cordifolia')
Carya_spp <- c('Carya', 'Carya cordiformis', 'Carya glabra', 'Carya ovata', 'Carya tomentosa')
Fraxinus_spp <- c('Fraxinus', 'Fraxinus americana', 'Fraxinus pennsylvanica', 'Fraxinus nigra')
Pinus_spp <- c("Pinus resinosa", "Pinus", "Pinus rigida") # Pinus strobus
Populus_spp <- c('Populus', 'Populus deltoides', 'Populus grandidentata', 'Populus tremuloides')
Prunus_spp <- c('Prunus', 'Prunus serotina', 'Prunus virginiana', 'Prunus pensylvanica')
Quercus_spp <- c('Quercus', 'Quercus (Red group)', 'Quercus (White group)',
                 'Quercus alba', 'Quercus bicolor', 'Quercus coccinea',
                 'Quercus montana', 'Quercus palustris', 'Quercus rubra',
                 'Quercus velutina')
...
reg_spp <- reg %>% left_join(., tlu_plants, by = c("TSN", "ScientificName")) %>%
  mutate(spp_grp = case_when(ScientificName %in% Acer_spp ~ "Acer spp. (maple)",
                             ...
                             ScientificName %in% Quercus_spp ~ "Quercus spp. (oak)",
                             ScientificName %in% Ulmus_spp ~ "Ulmus spp. (elm)",
                             ScientificName %in% c(Other_Native, Betula_spp, 
                                                   Pinus_spp, Populus_spp) ~ "Other Native spp.",
                             ScientificName %in% Exotic_spp ~ "Other Exotic spp.",
                             ScientificName %in% Subcanopy ~ "Subcanopy spp.",
                             TRUE ~ paste0(ScientificName, " (", CommonName, ")")))
...
shrubs <- do.call(joinMicroShrubData, args_vs) |> filter(SampleYear >= 2010) |> 
  filter(!ScientificName %in% 'Prunus virginiana')
#table(shrubs$ScientificName, shrubs$SampleYear)
shrub_tab <- as.data.frame(table(shrubs$ScientificName)) |> arrange(-Freq)
shrub_tab
cornus <- c("Cornus", "Cornus amomum", "Cornus racemosa", "Cornus rugosa")
other_native <- c("Alnus incana", "Alnus serrulata", "Aronia melanocarpa", 
                  "Amphicarpaea bracteata", "Aronia melanocarpa", "Gaylussacia baccata",
                  "Hamamelis virginiana", "Ilex mucronata", "Ilex verticillata", "Juniperus communis",
                  "Lindera benzoin",
                  "Lyonia ligustrina", "Rosa", "Sambucus nigra ssp. canadensis",
                  "Spiraea alba", "Unknown Shrub - 01",
                  "Vaccinium stamineum", "Zanthoxylum americanum", 'Viburnum lantanoides')
ericaspp <- c("Gaylussacia baccata", "Kalmia angustifolia", "Rhododendron viscosum",
              "Vaccinium angustifolium", "Vaccinium corymbosum", "Vaccinium pallidum")
corylus = c("Corylus americana", "Corylus cornuta")
other_exotic <- c("Berberis thunbergii",
                  "Campsis radicans", "Crataegus", "Lonicera japonica",
                  "Euonymus", 'Euonymus alatus', 'Ligustrum', "Viburnum lantana")
rubus <- c("Rubus", "Rubus allegheniensis", "Rubus idaeus", "Rubus occidentalis", "Rubus pensilvanicus")
viburnum <- c("Viburnum dentatum", "Viburnum lentago")
vitis <- c("Vitis", "Vitis aestivalis", "Vitis riparia", "Vitis labrusca")
lonicera <- c("Lonicera", "Lonicera - Exotic", "Lonicera morrowii")
parth <- c("Parthenocissus", "Parthenocissus quinquefolia")
natvines <- c(parth, vitis, "Smilax herbacea", "Smilax", "Smilax rotundifolia")
...
shrubs <- shrubs %>% 
  mutate(spp_grp = case_when(ScientificName %in% c(other_native, parth, corylus, cornus, viburnum) ~ "Other Native spp.",
                             #ScientificName %in% cornus ~ "Cornus spp. (dogwood)",
                             ScientificName %in% ericaspp ~ "Ericaceae spp. (heath family)",
                             ScientificName %in% rubus ~ "Rubus spp. (brambles)",
                             ScientificName %in% c(other_exotic, lonicera) ~ "Other Exotic spp.",
                             ...
                             ScientificName == "Celastrus orbiculatus" ~ "Celastrus orbiculatus (Asian bittersweet)",
                             ScientificName == "Toxicodendron radicans" ~ "Toxicodendron radicans (poison ivy)",
                             TRUE ~ paste0(ScientificName, " (", CommonName, ")")))
...
net_shrubs <- 
  ...
  theme_FHM()+
  scale_color_manual(values = c(
    "Celastrus orbiculatus (Asian bittersweet)" = "#FFB200",
    "Ericaceae spp. (heath family)" = "#00D8FF",
    #"Lonicera spp. (exotic honeysuckle)" = "#0070FF",
    "Other Exotic spp." = "#E5E204",
    "Native vine spp." = "#10AA0E",
    "Other Native spp." = "#828282",
    "Rhamnus cathartica (common buckthorn)" = "#FF7854",
    "Rhamnus frangula (glossy buckthorn)" = "#A31FFF",
    "Rosa multiflora (multiflora rose)" = "#BB3636",
    "Rubus spp. (brambles)" = "#DC91F6",#,
    "Toxicodendron radicans (poison ivy)" = "#0070FF"#,
    #"Viburnum spp. (arrowwood)" = "#84DE34"
  ), name = NULL) +
  scale_linetype_manual(values = c(
    "Celastrus orbiculatus (Asian bittersweet)" = "solid",
    "Ericaceae spp. (heath family)" = "solid",
    #"Lonicera spp. (exotic honeysuckle)" = "solid",
    "Other Exotic spp." = "dotdash",
    "Native vine spp." = "dotdash",
    "Other Native spp." = "dotdash",
    "Rhamnus cathartica (common buckthorn)" = "dotted",
    "Rhamnus frangula (glossy buckthorn)" = "solid",
    "Rosa multiflora (multiflora rose)" = "solid",
    "Rubus spp. (brambles)" = "dotdash",#,
    "Toxicodendron radicans (poison ivy)" = "solid"#,
    #"Viburnum spp. (arrowwood)" = "dotted"
...
frax <- do.call(joinTreeData, c(args_all, status = 'live')) |> filter(ScientificName %in% Fraxinus_spp)
head(frax)
...
frax_sum <- frax |> group_by(Plot_Name, cycle, ScientificName) |> 
  ...
  #mutate(FRAXSPP = FRAPEN + FRAAME + FRANIG) #SARA
  mutate(FRAXSPP = FRAPEN + FRAAME)
...
fraxspp[,4:8][is.na(fraxspp[,4:8])] <- 0
head(fraxspp)
