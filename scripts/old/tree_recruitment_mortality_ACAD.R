...
Acer_spp <- c('Acer', 'Acer saccharum', 'Acer saccharinum', 'Acer negundo')
...
tree_mr <- tree_mr |> mutate(cycle = case_when(SampleYear %in% cycle1 ~ 1, 
                                               SampleYear %in% cycle2 ~ 2,
                                               SampleYear %in% cycle3 ~ 3,
                                               SampleYear %in% cycle4 ~ 4))
...
net_ba <- 
  ggplot(mor_rec_ov_long %>% filter(unit_type == "BA"), 
    ...
    scale_color_manual(values = c("Abies balsamea (balsam fir)" = "#CF7742",
                                  "Acer rubrum (red maple)" = "#0EFF00",
                                  "Acer spp. (maple)" = "#338A06",
                                  "Betula spp. (birch)" = "#38A800",
                                  "Fagus grandifolia (American beech)" = "#FFAA00",
                                  "Fraxinus spp. (ash)" = "#A87000",
                                  "Larix laricina (tamarack)" = "#9778B1", 
                                  "Other Native spp." = "#828282",
                                  "Picea spp. (Spruce)" = "#C2037A",
                                  "Picea glauca (white spruce)" = "#099368",
                                  "Picea mariana (black spruce)" = "#004A92",
                                  "Picea rubens (red spruce)" = "#920000",
                                  "Pinus banksiana (Jack pine)" = "#00E6A9",  
                                  "Pinus resinosa (Red pine)" = '#F57A7A',
                                  "Pinus rigida (pitch pine)" = "#D24600", 
                                  "Pinus strobus (eastern white pine)" = "#B1AF78",
                                  "Populus spp. (poplar)" = "#F6EA42", 
                                  "Quercus rubra (northern red oak)" = "#C500FF",
                                  "Subcanopy spp." = "#FFBEE8",
                                  "Thuja occidentalis (eastern white cedar)" = "#53CEF2", 
...
net_stems <- 
  ggplot(mor_rec_ov_long %>% filter(unit_type == "stem"), 
         aes(x = cycle, y = log(net_total))) +
  geom_line(aes(color = spp_grp), size = 1.5) +
  labs(x = 'Cycle', y = "Stem Density (stems/ha)") +
  # geom_line(data = mor_rec_tot %>% filter(unit_type == "BA"), 
  #           aes(x = cycle, y = park_total), color = 'black') + 
  theme_FHM()+
  # may have to update for different parks
