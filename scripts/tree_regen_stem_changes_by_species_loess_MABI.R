...
Acer_spp <- c('Acer', 'Acer saccharinum', 'Acer negundo') #'Acer saccharum', 'Acer rubrum', )
...
Exotic_spp <- c('Acer platanoides',  
                'Aesculus hippocastanum', 'Ailanthus altissima',
                'Crataegus',  'Larix decidua',
...
trees <- trees %>% 
    mutate(spp_grp = case_when(ScientificName %in% Acer_spp ~ "Acer spp. (maple)",
                               ...
                               ScientificName %in% Pinus_spp ~ "Pinus spp. (pine - native)",
...
Other_Native <- c('Amelanchier',  'Amelanchier arborea', 'Amelanchier laevis',
                  ...
                  'Tilia americana', "Sorbus americana", "Sorbus decora",
                  'Pinus', 'Picea rubens',
                  'Platanus occidentalis', 
...
reg_spp <- reg %>% left_join(., tlu_plants, by = c("TSN", "ScientificName")) %>%
  mutate(spp_grp = case_when(ScientificName %in% Acer_spp ~ "Acer spp. (maple)",
                             ...
                             ScientificName %in% Prunus_spp ~ "Prunus spp. (cherry)",
                             ScientificName %in% Quercus_spp ~ "Quercus spp. (oak)",
                             ScientificName %in% c(Other_Native, Populus_spp, Carya_spp, Ulmus_spp) ~ "Other Native spp.",
                             ScientificName %in% Exotic_spp ~ "Other Exotic spp.",
                             ScientificName %in% Subcanopy ~ "Subcanopy spp.",
                             TRUE ~ paste0(ScientificName, " (", CommonName, ")")))
...
shrubs <- do.call(joinMicroShrubData, args_vs) |> filter(SampleYear >= 2010) |> 
  filter(!ScientificName %in% "Prunus virginiana") # not a shrub
table(shrubs$ScientificName, shrubs$SampleYear)

cornus <- c("Cornus", "Cornus amomum", "Cornus racemosa", "Cornus alternifolia")
...
sambucus <- c("Sambucus", "Sambucus nigra ssp. canadensis", "Sambucus nigra ssp. nigra", "Sambucus racemosa")

other_native <- c("Rhus typhina", "Ribes", "Ribes americanum", 
...
shrubs <- shrubs %>% 
  mutate(spp_grp = case_when(ScientificName %in% c(other_native) ~ "Other Native spp.",
                             ScientificName %in% cornus ~ "Cornus spp. (dogwood)",
                             ScientificName %in% rubus ~ "Rubus spp. (brambles)",
                             ScientificName %in% sambucus ~ "Sambucus spp. (elderberry)",
                             ScientificName %in% vitis ~ "Vitis spp. (grape)",
                             ScientificName == "Lonicera - Exotic" ~ "Lonicera spp. (exotic honeysuckle)",
...
net_shrubs <- 
  ...
  theme_FHM()+
  scale_color_manual(values = c(
    "Berberis vulgaris (common barberry)" = "#FF2525",
  ...
  scale_linetype_manual(values = c(
    "Berberis vulgaris (common barberry)" = "dotdash",
    "Cornus spp. (dogwood)" = "solid",
    "Lonicera spp. (exotic honeysuckle)" = "solid",
    "Other Native spp." = "dotdash",
    "Parthenocissus quinquefolia (Virginia creeper)" = "dotdash",
